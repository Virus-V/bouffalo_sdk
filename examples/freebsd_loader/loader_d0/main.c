...
uint32_t fit_img_decomp_node(void *fit, const char *name) {
  ...
  prop = fdt_get_property(fit, offset, "description", &len);
  printf("process lz4 decompress %s - 0x%x\r\n", prop->data, load_addr);
